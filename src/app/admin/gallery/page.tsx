  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 leading-tight">{t.admin.galleryPage.title}</h1>
          <p className="text-gray-500 font-medium mt-1 uppercase text-xs tracking-widest">{t.admin.galleryPage.subtitle}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sticky top-28">
            ...
            <div className="space-y-4">
              <div className="relative border-2 border-dashed rounded-xl border-gray-200 p-2 min-h-[150px] flex items-center justify-center overflow-hidden group">
                {newUrl ? (
                  ...
                  <div className="text-center p-4">
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase">{t.admin.galleryPage.imagePreview}</p>
                  </div>
                )}
              </div>

              <div>
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-tighter mb-1 block">{t.admin.galleryPage.imageAddress}</label>
                <input 
                  type="text" 
                  value={newUrl}
                  onChange={(e) => setNewUrl(e.target.value)}
                  placeholder={t.admin.galleryPage.pastePlaceholder}
              ...
              >
                ...
                <span>{uploading ? t.admin.galleryPage.addingBtn : t.admin.galleryPage.addBtn}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="flex items-center space-x-2">
             <h2 className="text-lg font-bold text-gray-900 italic">{t.admin.galleryPage.streamTitle}</h2>
             <span className="px-2 py-0.5 rounded-md bg-gray-100 text-gray-400 text-[10px] font-black">{images.length} {t.admin.galleryPage.photosCount}</span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
                  ...
                  >
                    ...
                    {imageErrors[image.id] && !isEditing && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                        ...
                        <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest leading-tight">{t.admin.galleryPage.brokenLink}</p>
                    ...
                    {isEditing && (
                      <div className="absolute inset-0 flex flex-col p-3 space-y-2">
                        ...
                        >
                           ...
                           <p className="absolute bottom-2 inset-x-0 text-center text-[8px] font-black text-white/50 uppercase pointer-events-none">{t.admin.servicesPage.clickToFocus}</p>
                        </div>
            ...
            {images.length === 0 && (
              <div className="col-span-full py-20 bg-gray-50 border-2 border-dashed border-gray-100 rounded-3xl flex flex-col items-center justify-center text-center">
                 ...
                 <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">{t.admin.galleryPage.emptyGallery}</p>
              </div>
            )}
          </div>
        </div>
